...
- **事件冲击**：发生游戏事件（如击杀、残局、爆头、多杀等）时，向调度器增加带 TTL 和线性衰减的上下文冲击，在未来数秒内以更高密度（约 0.8 ~ 1.8 秒一条）穿插发送。16类事件各自拥有一个不少于1000条的原文改编池；每次事件派发都只从对应池的全部条目随机选择，并通过事件历史与会话历史避免短期重复。原生小池与动态语义事件兜底均不再参与事件派发。
- **自然度控制**：
  - 最近已发文本窗口（64 条）硬去重；
...
- **事件池校验**：任一事件文件少于1000条、出现重复文本、换行、无效来源索引或无法回查的原文摘录时，运行时拒绝加载整套事件池，避免悄悄落入无关弹幕。
