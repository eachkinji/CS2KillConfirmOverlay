...
从 4.5.1.44 起，应用内置一套原版图标（含公共特效）和一套斯沃特保卫者语音，首次安装即可使用。基础资源位于 `SourceAssets/GameStyles/crossfire/iconpacks/default` 和 `soundpacks/crossfire_swat_gr`。其余 CF 套装继续外置；旧序列帧不进入安装包。外部仓库仍提供完整的 8 套原有图标和 11 套语音及其他素材套装。
...
选择“原版”叠加或缺少事件的对应原图时，可直接读取内置原版包。导入同名基础包会覆盖该选择的资源来源；删除导入版本后恢复内置基础包。

悬浮窗的图标和语音选择栏最后一项为“添加更多”。选择它会保留原来的包选择，并打开高级设置中当前游戏的图标或音频包页面。CF 和瓦罗兰特页面在“导入”左侧提供对应的下载按钮；列表下方虚线区域支持拖入一个或多个 ZIP 导入。
...
最终 MSIX 构建检查只允许上述两套基础 CF 资源，并逐文件检查内容与源文件的 SHA-256；其余 CF 素材和旧序列帧仍禁止混入。
